package com.RimAirV3.VersionSpring.controller;
...
import com.RimAirV3.VersionSpring.services.AirportService;
...
public class MainController {
    ...
    public String searchFlight(@RequestParam("departureAirport") int departureAirport,
        ...
        if (departureAirport == destinationAirport) {
            model.addAttribute("AirportError", "L'aéroport de départ et de destination ne peut pas être le même !");
            model.addAttribute("airports", airportService.getAllAirports());
            return "searchFlight";
        }
        ...
        if (flight != null) {
            ...
            for (Passenger p : passengers) {
                if (p.getPassportNumber().equals(passportNumber)) {
                    passenger = passengerService.getPassengerById(p.getPassengerId());
